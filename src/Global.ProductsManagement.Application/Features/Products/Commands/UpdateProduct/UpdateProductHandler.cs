...
namespace Global.ProductsManagement.Application.Features.Products.Commands.UpdateProduct
{
    public class UpdateProductHandler : IRequestHandler<UpdateProductCommand, UpdateProductResponse>
    {
        ...
        readonly ILogger<UpdateProductHandler> _logger;
        ...
        public async Task<UpdateProductResponse> Handle(UpdateProductCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                if (_productRepository.Exists(product.Name, product.CategoryId, product.BrandId, product.Id))
                {
                    ...
                    return _notificationsHandler
                        ...
                        .ReturnDefault<UpdateProductResponse>();
                }

                _productRepository.Update(product);
                await _unitOfWork.CommitAsync();

                var @event = _mapper.Map<UpdatedProductEvent>(product);

                await _productProducer.SendUpdatedEventAsync(@event);
            ...
            {
                _logger.LogError(ex, "An error occurred when trying to update the product: {Exception}", ex.Message);
